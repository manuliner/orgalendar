  ...
  <v-dialog v-model="openDialog" width="500">
    <v-card>
      ...
      <v-card-text>
        ...
        <!-- <v-row>
          <v-btn icon @click="numberAddUserDialog++">
      ...
      <v-card-actions>
        <v-btn text @click="closeAddUserDialogFail"> Abrechen </v-btn>
        <v-spacer />
        <v-btn :disabled="!valid" @click="closeAddUserDialogSuccess">
          Hinzufügen
        </v-btn>
      </v-card-actions>
    </v-card>
  ...
  methods: {
    closeAddUserDialogSuccess() {
      ...
      for (let i = 1; i < this.numberAddUserDialog + 1; i++) {
        this.createUser(
          this.$refs["addUser" + i][0]._data.userName,
